tpsco<- function (par, fn, gr = NULL, ..., lower=-1, upper=1,
  ...
  con <- list(trace = 1, fnscale = 1, maxit = 1000L, maxf = Inf,
              ...
              trace.stats = FALSE, type = "SPSO2007",SCOND=TRUE,m=5,iter=10,GU=TRUE,r=1,n=10)
  nmsC <- names(con)
  ...
  inter2<-function(x,y,w,p){
    z<-matrix(rep(p,p.s),ncol=p.s,nrow=npar)
    m<-which.min(y[[3]])
    g<-matrix(rep(y[[1]][,m],p.s),nrow=npar,ncol=p.s)
    g[,m]<-x[[1]][,m]
    x[[2]]<-w*x[[2]]+runif(npar,0,p.c.p)*(y[[1]]-x[[1]])+runif(npar,0,p.c.g)*(z-x[[1]])+runif(npar,0,p.c.g)*(g-x[[1]])
  ...
  while (stats.iter<p.maxit && stats.feval<p.maxf && error>p.abstol &&
    ...
    if (!p.vectorize) {
      ...
      for (i in index) {
        ...
        if(p.gu){
          X_G<-NULL
          pd<-pd*(p.maxit-stats.iter)/p.maxit
          
          X_G<-Gu(X[,i],-pd,pd,p.n)
          X_GF<-apply(X_G,2,fn1)
...
pso<-tpsco(rep(NA,30),test_f,lower=-100,upper=100,control=list(maxit=100,trace=0,REPORT=TRUE,SCOND=TRUE,GU=TRUE,m=10,r=10,n=10))